...
client.on('credentials-updated', () => {
   ...
   fs.writeFileSync('./melokgaming.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./melokgaming.json') && client.loadAuthInfo('./melokgaming.json')
client.connect('succes...');
	...
	client.on('message-new', async (mek) => {
		...
		try {
			...
			const groupAdmins = isGroup ? getGroupAdmins(groupMembers) : ''
           	const ftoko = { key: { fromMe: false, participant: `6281241405996@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync('./src/radit.JPG') }, "title": `Subrek ItsAra`, "productImageCount": 9999 }, "businessOwnerJid": `6281332610191@s.whatsapp.net`}}}
...
custom(radit(prefix), text, tescuk, cr)
...
const ara = fs.readFileSync('./src/sticker/melokgaming2.webp');
		...
		if (budy.includes(`jadibot`)) {
                  reply(`di usahakan baca salam seperti Assalamualaikum 😊`)
const ara = fs.readFileSync('./src/sticker/melokgaming4.webp');
                  client.sendMessage(from, ara, MessageType.sticker, {quoted: mek})
                  }
		if (budy.includes(`Apakah`)) {
                  reply(`mungkin iya  😊`)
const ara = fs.readFileSync('./src/sticker/melokgaming4.webp');
                  client.sendMessage(from, ara, MessageType.sticker, {quoted: mek})
                  }
if (budy.includes(`apakah`)) {
                  reply(`apa tidak kah ${pusname}   😊`)
const ara = fs.readFileSync('./src/sticker/melokgaming4.webp');
                  client.sendMessage(from, ara, MessageType.sticker, {quoted: mek})
                  }
if (budy.includes(`Apa`)) {
                  reply(`gue gatau 😊`)
const ara = fs.readFileSync('./src/sticker/melokgaming6.webp');
                  client.sendMessage(from, ara, MessageType.sticker, {quoted: mek})
                  }

		if (budy.includes(`Kapan`)) {
                  reply(`besok 😊`)
const ara = fs.readFileSync('./src/sticker/melokgaming5.webp');
                  client.sendMessage(from, ara, MessageType.sticker, {quoted: mek})
                  }
